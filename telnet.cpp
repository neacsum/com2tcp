    ...
    switch (state) {
      ...
      case stSubCode:
        switch (ch) {
          ...
          case cdSE:
            printf("  ");
            for (BYTE_vector::const_iterator it = params.begin() ; it != params.end() ; it++)
              printf("%u ", (unsigned)*it);
            printf("SE\n");
...
void TelnetProtocol::SendOption(BYTE c, BYTE opt)
{
  BYTE buf[3] = {cdIAC, c, opt};

  printf("SEND: %s %u\n", code2name(c), (unsigned)opt);

  SendRaw(buf, sizeof(buf));
}

void TelnetProtocol::SendSubNegotiation(int opt, const BYTE_vector &par)
{
  SendOption(cdSB, (BYTE)opt);

  printf("  ");
  for (BYTE_vector::const_iterator i = par.begin() ; i != par.end() ; i++) {
    BYTE b = *i;
